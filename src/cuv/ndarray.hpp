 ...
 * Example:
 * @code
 * ndarray<...> v(extents[5][6][7]); // 3-dimensional ndarray
 * @endcode
 */
 ...
 * Example:
 * @code
 * ndarray_view<...> v(indices[index_range(1,3)][index_range()], other_ndarray);
 * // or, equivalently
...
public:
    ...
    typedef unsigned int size_type; ///< type of shapes of the ndarray
    typedef int index_type; ///< type of indices in ndarray
...
public:
    ...
    typedef ndarray_info<M, L> info_type; ///< type of shape info struct
    typedef ndarray_view<V, M, L> view_type; ///< type of views on this ndarray
...
protected:
     ...
     * are interpreted as relative to the (strided) sub-ndarray we're
     * referring to.
     *
...
public:
    ...
    /// @return the ndarray info struct (const)
    const info_type& info() const {
        return m_info;
    }

    /// @return the ndarray info struct
    info_type& info() {
        return m_info;
    }
     ...
     * @param i0 index for a 1-dimensional ndarray
    ...
    /**
     * construct ndarray from ndarray of other memory space
    ...
    /**
     * construct ndarray from ndarray of same memory space
    ...
    /**
     * @name assigning other values to a ndarray object
     * @{
     */
...
public:
     ...
     *
     * @param o a ndarray of another memory space type
     */
    template<class OM>
     ...
     *
     * @param o a ndarray_view of another memory space type
     */
    template<class OM>
     ...
     *
     * @warning if a dimension has size 1, the resulting ndarray has fewer dimensions than the original one.
     *
     * @warning most operations in CUV on ndarrays currently only work
     *          if the sub-ndarray is a connected area in memory.  Basically this
     *          means that you can only slice in the first dimension which has
     *          size>1.
     *
     * @param idx  the indices of the sub-ndarray
     * @param o   the original ndarray
     *
     * Example:
     * @code
...
struct switch_value_type {
    typedef ndarray<NewVT, typename Mat::memory_space_type, typename Mat::memory_layout_type> type; ///< new ndarray type after switch
};
...
struct switch_memory_layout_type {
    typedef ndarray<typename Mat::value_type, typename Mat::memory_space_type, NewML> type; ///< new ndarray type after switch
};
...
struct switch_memory_space_type {
    typedef ndarray<typename Mat::value_type, NewMS, typename Mat::memory_layout_type> type; ///< new ndarray type after switch
};
...
/**
 * print a dev ndarray to a stream (copying to host first)
 *
 * @param o the stream
 * @param t the ndarray
 */
...
/**
 * print a host ndarray to a stream
 *
 * @param o the stream
 * @param t the ndarray
 */
